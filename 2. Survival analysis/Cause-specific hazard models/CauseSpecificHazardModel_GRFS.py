...
def compet_risk_converter_median_splitter_GRFS(input_df, treat_as_continuous):
    INF = 10**9
    df = input_df.copy()
    ...
    df['GRFS_SUM'] = df[[OUTCOME_COLS['RFS']['event'], 'severe_aGVHD_simplified', 'severe_cGVHD_simplified']].sum(axis = 1)
    df['Event_GRFS'] = np.where(df['GRFS_SUM'] > 0, 1, 0)
    df['Time_to_GRFS'] = np.where(
        df['Event_GRFS'] == 1,
        np.where(
            df["aGVHD_pre35"] == 1,
            df[[OUTCOME_COLS['cGVHD']['time'], OUTCOME_COLS['RFS']['time']]].min(axis = 1),
            df[[OUTCOME_COLS['aGVHD']['time'], OUTCOME_COLS['cGVHD']['time'], OUTCOME_COLS['RFS']['time']]].min(axis = 1)
        ),
        df['MAX_FOLLOW_UP']
    )
    
    # candidate post-d35 times (only if event truly happened)
    df["t_GRFS_post"] = [candidate_time(t, e) for t, e in zip(df['Time_to_GRFS'], df['Event_GRFS'])]
    ...
    df["t_first"] = df[["t_GRFS_post", "t_death_post"]].min(axis=1)
    df["t_first"] = np.minimum(df["t_first"], df["MAX_FOLLOW_UP"])

    df = assign_first_event(
        df,
        event_order=[
            ("GRFS_first", "t_GRFS_post"),
            ("death_first",   "t_death_post"),
        ]
    )
    first_cols = ["GRFS_first", "death_first"]
...
def runner_coxph(OC_intput, VC_input, input_col, treat_as_continuous):
    ...
    analysis_type = f'GRFS({aGVHD_part}_{cGVHD_part})_Relapse_Death'
...
d35_input_scores['severe_cGVHD_simplified'] = np.where(d35_input_scores['cGVHD_grading_by_Katja'].isin(severe_cGVHD_cats), 1, 0)

for treat_as_continuous in [True, False]:
    print('treat_as_continuous', treat_as_continuous)
    d35_cohort_OC, d35_cohort_VC = compet_risk_converter_median_splitter_GRFS(d35_input_scores, treat_as_continuous)
    
    for input_col in ['GRFS_first']:
