...
severe_cGVHD_cats = [int(element) for element in str(sys.argv[3]).split(',')]
...
def compet_risk_converter_median_splitter_severe_GVHD(input_df, GVHD_merged, treat_as_continuous, split_cGVHD_cats):
    INF = 10**9
    df = input_df.copy()
    ...
    if (1 in severe_cGVHD_cats) and (2 in severe_cGVHD_cats):
    ...
    if GVHD_merged:
        df["t_GVHD_post"] = np.minimum(df["t_aGVHD_post"], df["t_cGVHD_post"])

        df["t_first"] = df[["t_relapse_post", "t_GVHD_post", "t_death_post"]].min(axis=1)
        ...
        df = assign_first_event(
            ...
            event_order=[
                ("relapse_first", "t_relapse_post"),
                ("gvhd_first",    "t_GVHD_post"),
                ("death_first",   "t_death_post"),
            ]
        )
        first_cols = ["relapse_first", "gvhd_first", "death_first"]

    else:
        df["t_first"] = df[["t_relapse_post", "t_aGVHD_post", "t_cGVHD_post", "t_death_post"]].min(axis=1)
        df["t_first"] = np.minimum(df["t_first"], df["MAX_FOLLOW_UP"])
        ...
        first_cols = ["relapse_first", "agvhd_first", "cgvhd_first", "death_first"]

        if (1 in severe_cGVHD_cats) and (2 in severe_cGVHD_cats):
            if split_cGVHD_cats == 'cGVHD_Split':
                df["t_first"] = df[["t_relapse_post", "t_aGVHD_post", "t_cGVHD_1_post", "t_cGVHD_2_post", "t_death_post"]].min(axis=1)
                df["t_first"] = np.minimum(df["t_first"], df["MAX_FOLLOW_UP"])
                ...
                first_cols = ["relapse_first", "agvhd_first", "cgvhd_1_first", "cgvhd_2_first", "death_first"]       
    ...
    d35_cohort_OC = df[df['Cohort'] == 'OC'].copy()
    # print('OC', len(d35_cohort_OC))
    d35_cohort_VC = df[df['Cohort'] == 'VC'].copy()
...
def runner_coxph(OC_intput, VC_input, input_col, treat_as_continuous):
    ...
    for cohort in ['OC', 'VC']:
        ...
        if input_col in ['agvhd_first', 'gvhd_first']:
    ...
    if len(severe_cGVHD_cats) == 1:
        cGVHD_part = 'cGVHD' + str(severe_cGVHD_cats[0])
    elif len(severe_cGVHD_cats) > 1:
        cGVHD_part = 'cGVHD' + ''.join([str(element) for element in severe_cGVHD_cats])
    ...
    analysis_type = f'{aGVHD_part}_{cGVHD_part}_Relapse_Death'
...
d35_input_scores['severe_aGVHD_simplified'] = np.where(d35_input_scores[OUTCOME_COLS['aGVHD']['event']].isin(severe_aGVHD_cats), 1, 0)
d35_input_scores['severe_cGVHD_simplified'] = np.where(d35_input_scores[OUTCOME_COLS['cGVHD']['event']].isin(severe_cGVHD_cats), 1, 0) 
if (1 in severe_cGVHD_cats) and (2 in severe_cGVHD_cats):
    d35_input_scores['cGVHD_1_happening_event'] = np.where(d35_input_scores[OUTCOME_COLS['cGVHD']['event']] == 1, 1, 0)
    d35_input_scores['cGVHD_1_happening_time'] = np.where(d35_input_scores['cGVHD_1_happening_event'] == 1, d35_input_scores[OUTCOME_COLS['cGVHD']['time']], d35_input_scores['MAX_FOLLOW_UP'])
    d35_input_scores['cGVHD_2_happening_event'] = np.where(d35_input_scores[OUTCOME_COLS['cGVHD']['event']] == 2, 1, 0)
    d35_input_scores['cGVHD_2_happening_time'] = np.where(d35_input_scores['cGVHD_2_happening_event'] == 1, d35_input_scores[OUTCOME_COLS['cGVHD']['time']], d35_input_scores['MAX_FOLLOW_UP'])

print('gvhd_merged = True')
for gvhd_merged in [True]:
    for treat_as_continuous in [True, False]:
        print('treat_as_continuous', treat_as_continuous)
        d35_cohort_OC, d35_cohort_VC = compet_risk_converter_median_splitter_severe_GVHD(d35_input_scores, gvhd_merged, treat_as_continuous, 'cGVHD_NotSplit')
        
        for input_col in ['gvhd_first', 'relapse_first']:
            print(input_col)
            runner_coxph(d35_cohort_OC, d35_cohort_VC, input_col, treat_as_continuous)

print('gvhd_merged = False, cGVHD not split')
for gvhd_merged in [False]:
    for treat_as_continuous in [True, False]:
        print('treat_as_continuous', treat_as_continuous)
        d35_cohort_OC, d35_cohort_VC = compet_risk_converter_median_splitter_severe_GVHD(d35_input_scores, gvhd_merged, treat_as_continuous, 'cGVHD_NotSplit')
        
        for input_col in ['agvhd_first', 'cgvhd_first']:
            print(input_col)
            runner_coxph(d35_cohort_OC, d35_cohort_VC, input_col, treat_as_continuous)

if (1 in severe_cGVHD_cats) and (2 in severe_cGVHD_cats):
    print('gvhd_merged = False, cGVHD split')
    for gvhd_merged in [False]:
        for treat_as_continuous in [True, False]:
            print('treat_as_continuous', treat_as_continuous)
            d35_cohort_OC, d35_cohort_VC = compet_risk_converter_median_splitter_severe_GVHD(d35_input_scores, gvhd_merged, treat_as_continuous, 'cGVHD_Split')
